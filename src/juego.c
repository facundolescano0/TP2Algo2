				       ...
				       const char *nombre3)
{
	...
	if(jugador == JUGADOR1){
		...
		for(int i = 0; i<MAX_POKEMONES;i++){
			if(i==OPONENTE)
				juego->jugador2->recursos[i].pokemon = pokemones[i];
			else
...
int determinar_puntos(const struct ataque *ataque1, const struct ataque *ataque2){
...
resultado_jugada_t determinar_resultado(juego_t *juego,
				const struct ataque *ataque1, const struct ataque *ataque2)
{
...
resultado_jugada_t fallo_ataques(const struct ataque *ataque1,const struct ataque *ataque2)
{
	resultado_jugada_t resultado;
	resultado.jugador1 = ATAQUE_ERROR;
	resultado.jugador2 = ATAQUE_ERROR;
...
{	
	...
	struct ataque_cargar ataque_cargar1 = verificar_jugada(jugador1->recursos,jugada_jugador1);
	struct ataque_cargar ataque_cargar2 = verificar_jugada(jugador2->recursos,jugada_jugador2);
